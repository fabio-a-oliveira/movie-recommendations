...
# script HX9_MovieLens_Main.R needs to be sourced to save file "HX9_MovieLens.RData" to work directory before kniting this report!

load(paste(getwd(),"/HX9_MovieLens.RData", sep=""))
...
data.frame(Object = c("train", "test", "validation"),
  ...
  transmute(Object,
            ...
            "Number of Users (% of total)" = paste0(ratings," (",proportion_users,"%)"),
            "Number of Movies (% of total)" = paste0(ratings," (",proportion_movies,"%)")) %>% 
...
df %>% 
  ...
  annotate(geom = "label", label = paste("lambda = ", df$lambda[which.min(df$R)], "\n RMSE = ", round(min(df$R),5)), x = 3.2, y = .94402) +
  annotate(geom = "curve", curvature = .1, color = 'red', arrow = arrow(length = unit(2, 'mm')), xend = 2.5, yend = .943995, x = 3.1, y = .944014)
...
The application of this model in the _test_ set gives the following results:
...
Again, we see that regularization does not introduce a great gain in performance, which is explained by the fact the fact that we are using a massive dataset and that ratings from users that are not very active do not account for a significant portion of the total ratings.
